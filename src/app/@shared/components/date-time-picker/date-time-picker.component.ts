...
export class DateTimePickerComponent {
  ...
  initTime() {
    ...
    this.hours = Array.from({ length: 12 }, (_, i) => `${i + 1}`.padStart(2, '0'));
    this.minutes = Array.from({ length: 60 }, (_, i) => `${i}`.padStart(2, '0'));
    this.seconds = Array.from({ length: 60 }, (_, i) => `${i}`.padStart(2, '0'));
   ...
   currentPresentTime(){
    ...
    this.currentTime = `${(hours % 12 || 12).toString().padStart(2, '0')}:${minutes
      .toString()
  ...
  updateCurrentTime() {
    ...
    this.selectedHour = (hours % 12 || 12).toString().padStart(2, '0');
    this.selectedMinute = minutes.toString().padStart(2, '0');
    this.selectedSecond = seconds.toString().padStart(2, '0');
    this.selectedAmPm = amPm;
    this.selectedTime = `${this.selectedHour}:${this.selectedMinute}:${this.selectedSecond} ${this.selectedAmPm}`;
  }
   updateSelectedTime(event:Event) {
    ...
    if (this.selectionMode === 'range') {
      if (!this.selectedStartDate && !this.selectedEndDate) {
        // If both start and end dates are null
        const dateTimerange = `${this.formatCurrentRangeDate(currentDate)} - ${this.formatCurrentRangeDate(currentDate)} ${this.selectedTime}`;
        this.dateRangeControl.setValue(dateTimerange);
        this.onChange(dateTimerange);
      } else if (this.selectedStartDate && this.selectedEndDate) {
        // If both start and end dates are selected
        const dateTimerange = `${this.formatCurrentRangeDate(this.selectedStartDate)} - ${this.formatCurrentRangeDate(this.selectedEndDate)} ${this.selectedTime}`;
        this.dateRangeControl.setValue(dateTimerange);
        this.onChange(dateTimerange);
      }
    } else {
      if (!this.selectedStartDate) {
        ...
        const dateTime = `${this.formatCurrentRangeDate(currentDate)} ${this.selectedTime}`;
        this.dateRangeControl.setValue(dateTime);
        this.onChange(dateTime);
      } else {
        // If start date is selected
        const dateTime = `${this.formatCurrentRangeDate(this.selectedStartDate)} ${this.selectedTime}`;
        this.dateRangeControl.setValue(dateTime);
        this.onChange(dateTime);
      }
    }
  }
  formatCurrentRangeDate(date:any){
  ...
  getDayClass(day: Date): string {
    const baseClass =
      'flex h-[40px] w-[40px] items-center justify-center text-sm';
    ...
    if (isToday) {
      classes.push(
        'bg-gray-500 dark:bg-gray-600 border border-lime-300 text-white rounded-full'
      );
    }
  ...
  private updateInput() {
    ...
    if (this.selectionMode === 'date' && this.selectedStartDate) {
      ...
      const dateTime = `${dateStr} ${this.selectedTime}`;
      this.dateRangeControl.setValue(dateTime);
      this.onChange(dateTime);
    } else if (this.selectionMode === 'range') {
      // Range mode
      if (this.selectedStartDate && this.selectedEndDate) {
        ...
        const dateTimerange = `${startStr} - ${endStr} ${this.selectedTime}`;
        this.dateRangeControl.setValue(dateTimerange);
        this.onChange(`${startStr} - ${endStr}  ${this.selectedTime}`);
      } else if (this.selectedStartDate) {
        const selectedDateTime = `${formatDate(this.selectedStartDate)} ${this.selectedTime}`;
        this.dateRangeControl.setValue(selectedDateTime);
